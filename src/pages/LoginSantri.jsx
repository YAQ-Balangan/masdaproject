// src/pages/LoginSantri.jsx
import React from "react";
import { motion, AnimatePresence } from "framer-motion";
import { Search, ArrowLeft } from "lucide-react";

export default function LoginSantri({ navigateTo, loginInputSantri, setLoginInputSantri, loginError, setLoginError, handleLoginSantri }) {
    return (
        <motion.div key="ls" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center min-h-screen p-6">
            <div className="w-full max-w-md p-10 bg-white rounded-[3rem] shadow-[0_20px_50px_rgb(0,0,0,0.1)] relative border border-slate-50">
                <button onClick={() => navigateTo("selection")} className="absolute top-8 left-8 p-2 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-full transition-colors">
                    <ArrowLeft size={24} />
                </button>
                ...
                <div className="space-y-4">
                    ...
                    <AnimatePresence>
                        {loginError && (
                            <motion.p initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="text-red-500 text-xs font-bold text-center">
                                {loginError}
                            </motion.p>
                        )}
                    </AnimatePresence>
